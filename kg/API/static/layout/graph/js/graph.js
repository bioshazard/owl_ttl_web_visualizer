...
function addInfoCard(node){
    ...
    $.ajax({
        type: 'GET',
        url: 'http://127.0.0.1:5005/node_info/'+node.intra_id,
        dataType: 'json',
        success: function(data) {
            console.log("success")
            console.log(data)
            if (data.length == 0){
                console.log('error for intra id:' + node.intra_id)
                return 0
            }
            ...
            pp2.appendChild(document.createTextNode(data["summary_explanation"].slice(0, 300) + "..."))
            ...
            pp5.appendChild(document.createTextNode("Veridica"))
            ...
            console.log(element)
            //element.insertBefore(p, element.lastChild)
